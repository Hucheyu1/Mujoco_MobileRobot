...
class DiffDrivePointGoalEnv(gym.Env):
    ...
    def __init__(
        ...
        self.robot = Robot(
            self.model, self.data, robot_body_name="pusher1", actuator_names=["forward", "turn"], max_v=1.0, max_w=1.0
        )
        ...
        # 相对于目标的 dx, dy、距离 dist、航向误差 angle\_err、以及上一时刻的动作
    ...
    def _get_obs(self):
        pos_xy, yaw = self._get_pose()  # 获取当前位置和航向角
        d = self.goal - pos_xy  # 计算到目标的位移向量
        dist = float(np.linalg.norm(d))  # 欧氏距离

        # 计算目标方位角 (bearing) 与当前航向 (yaw) 的偏差
    ...
    def step(self, action):
        ...
        # reward：
        # progress：如果靠近目标，奖励为正
        # shaping：越靠近目标，每一步的基础分越高
        # control_penalty: 惩罚大幅度动作，防止电机过热或剧烈抖动。
        # smooth_penalty: 惩罚当前动作与上一次动作的差异，鼓励平滑驾驶。
        # angle_penalty: 惩罚机器人没有正对目标的行为。
        # time_penalty: 每走一步扣 0.05 分，逼迫机器人寻找最短路径，而不是在原地转圈

...
def plot_predictions(all_preds, all_labels_x, all_labels_u, plot_idx, save_dir, epoch=None):
    ...
    # plt.show()
    fig.savefig(save_dir + f"/plot_predictions_{epoch}.png", dpi=300)
    plt.close(fig)


def plot_contour(all_preds, all_labels_x, all_labels_u, plot_idx, save_dir, epoch=None):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    n_rows = len(plot_idx)
    n_cols = 3
    ...
    plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1, hspace=0.4, wspace=0.4)
    # plt.show()
    fig.savefig(save_dir + f"/plot_predictions_contour_{epoch}.png", dpi=300)
    plt.close(fig)
